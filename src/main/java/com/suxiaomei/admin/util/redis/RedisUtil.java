...
public class RedisUtil {  
    ...
    public static long sSetAndTime(RedisTemplate<String,Object> redisTemplate,String key,long time,Object...values) {  
        try {  
            ...
            if(time>0) expire(redisTemplate,key, time);  
            return count;  
        } catch (Exception e) {  
            e.printStackTrace();  
            return 0;  
     ...
     * @return
     */
    public static boolean setList(RedisTemplate<String, Object> redisTemplate,String key,List list){
    	JSONArray dictionaryListArray = JSONArray.fromObject(list);
		return set(redisTemplate,key, dictionaryListArray.toString());
    }
    
    public static <T> List<T> getList(RedisTemplate<String, Object> redisTemplate,String key,T t){
    	try {
    		String dictionaryStr = RedisUtil.get(redisTemplate, key).toString();
        	JSONArray jsonArray = JSONArray.fromObject(dictionaryStr);
        	@SuppressWarnings("unchecked")
    		List<T> list = JSONArray.toList(jsonArray, t, new JsonConfig());
            return list;
		} catch (Exception e) {
		}
    	return null;
