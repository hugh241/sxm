...
public enum Code {
	...
	CANNOTDELETEIDENTIFICATIONBUSINESS(478,"认证未到期，不能删除"),ITEMSNOTCOMFIRM(479,"存在未确认项目"),BACKGROUNDCHECKERROR(480,"背调接口返回错误"),
	EDUCATIONINFO(481,"培训分数不合格"),ISOCGOODSNUMSLACK(482,"产品数量不足"),RUNNINGORDEREXIST(483,"当前时间段内存在其他档期"),
	RUNNINGCONTRACTEXIST(484,"存在进行中的合同,不能移出"),TRAINSCORENOTPASS(485,"分数不合格，不能认证");
	// 成员变量
    private int code;
